...
class ConditionsCategoryTest extends FunSuite {
  private val oc = ConstraintSet(ImageQuality.Preset.PointOne, CloudExtinction.Preset.PointThree, SkyBackground.Darkest, WaterVapor.VeryDry, ElevationRange.ByAirMass.Default)
  ...
  test("testMultiple") {
    ...
    assert(!cat.matches(ConstraintSet(ImageQuality.Preset.PointOne, CloudExtinction.Preset.Zero, SkyBackground.Darkest, WaterVapor.VeryDry, ElevationRange.ByAirMass.Default)))
    assert(!cat.matches(ConstraintSet(ImageQuality.Preset.OnePointZero, CloudExtinction.Preset.PointThree, SkyBackground.Darkest, WaterVapor.VeryDry, ElevationRange.ByAirMass.Default)))
  }
  ...
  test("testAll") {
    ...
    assert(cat.matches(oc))
    assert(!cat.matches(ConstraintSet(ImageQuality.Preset.PointOne, CloudExtinction.Preset.PointThree, SkyBackground.Dark, WaterVapor.VeryDry, ElevationRange.ByAirMass.Default)))
    assert(!cat.matches(ConstraintSet(ImageQuality.Preset.PointOne, CloudExtinction.Preset.PointThree, SkyBackground.Darkest, WaterVapor.Dry, ElevationRange.ByAirMass.Default)))
  }
  ...
  test("testCanObserveAll") {
    ...
    val oc = ConstraintSet(ImageQuality.Preset.PointOne, CloudExtinction.Preset.OnePointZero, SkyBackground.Dark, WaterVapor.Wet, ElevationRange.ByAirMass.Default)
