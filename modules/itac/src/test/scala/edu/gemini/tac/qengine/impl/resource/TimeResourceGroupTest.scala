...
class CompositeTimeRestrictionResourceTest extends ItacSuite {
  ...
  private val lgsBin = TimeRestriction("lgs", Time.fromHoursBounded(1)) {
    (_, obs, _) => obs.lgs
  }

  // 10% of 10 hours = 1 hr = 60 min
  private val resWV60min  = TimeRestrictionResource(wvBin, Time.fromHoursBounded(10))
  ...
  private def mkProp(wv: WaterVapor, lgs: Boolean): Proposal =
    Proposal(ntac, site = Site.GS, obsList = List(ItacObservation(target, conds(wv), Time.fromHoursBounded(10), lgs)))
  ...
  test("testReserveWv") {
    ...
    grp.reserve(block, Fixture.emptyQueue) match {
      case Right(newGrp) => {
        ...
        assertEquals(Time.fromMinutesBounded(45), res1.remaining)
        assertEquals(Time.fromMinutesBounded(60), res2.remaining)
      }
      case _ => fail("failed")
    }
  }
  ...
  test("testReserveLgs") {
    ...
    grp.reserve(block, Fixture.emptyQueue) match {
      case Right(newGrp) => {
        ...
        assertEquals(Time.fromMinutesBounded(60), res1.remaining)
        assertEquals(Time.fromMinutesBounded(45), res2.remaining)
      }
      case _ => fail("failed")
    }
  }
