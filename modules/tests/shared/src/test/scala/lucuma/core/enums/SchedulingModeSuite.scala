...
final class SchedulingModeSuite extends FunSuite:
  ...
  private val activations =
    List(TooActivation.None, TooActivation.Rapid, TooActivation.Interrupting)
  ...
  test("every Target of Opportunity, and nothing else, must be Uninterruptible"):
    assertEquals(
      activations.filter(_.requiresUninterruptible),
      List(TooActivation.Rapid, TooActivation.Interrupting)
    )
    assertEquals(activations.filter(_.requiresUninterruptible), activations.filter(_.isToo))

  // Five of the nine pairings are legal.  The four that are not are exactly the
  // two Target of Opportunity activations over the two modes that permit
  // interruption.
