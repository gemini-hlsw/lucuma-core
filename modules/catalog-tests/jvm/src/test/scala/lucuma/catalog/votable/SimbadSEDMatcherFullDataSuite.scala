...
/**
 * Comprehensive test suite using the full match_sed_test.dat dataset (8000+ entries).
 * Tests SED matching against the complete Python reference dataset.
 */
class SimbadSEDMatcherFullDataSuite extends FunSuite:
  ...
  test("comprehensive statistics") {
    ...
    val byOtype = matchResults.filter(_.sed.isEmpty).groupBy(_.entry.otype).view.mapValues(_.length).toMap
    ...
    // Basic sanity checks
    assert(matched > total / 2, s"Expected at least 50% match rate, got ${matched * 100 / total}%")
  }
  ...
  test("galaxy type coverage") {
    ...
    println(s"Galaxy types: ${galaxyResults.length} total, $galaxyMatched matched (${galaxyMatched * 100 / galaxyResults.length}%)")
  ...
  test("quasar type coverage") {
    ...
    println(s"Quasar types: ${quasarResults.length} total, $quasarMatched matched (${quasarMatched * 100 / quasarResults.length}%)")
  ...
  test("spectral type parsing coverage") {
    ...
    println(s"Successfully parsed and matched: ${spectralTypeParsed.length} (${spectralTypeParsed.length * 100 / withSpectralType.length}%)")
  }
