...
class GoaCodecsSuite extends CatsEffectSuite:
  ...
  test("decode sidereal response JSON"):
    loadResource("goa/sidereal-response.json").map: json =>
      ...
      assertEquals(first.utDateTime, Some(LocalDateTime.of(2024, 1, 15, 6, 42, 29, 400000000)))
  ...
  test("LocalDateTime decoder handles various formats"):
    val json1 =
      """{"name": "test.fits", "instrument": "GMOS-N", "observation_type": "OBJECT", "ut_datetime": "2024-01-15 06:42:29.400000"}"""
    val result1 = decode[GoaSummaryRecord](json1.stripMargin)
    assert(result1.isRight)
    assertEquals(
      result1.toOption.get.utDateTime,
      Some(LocalDateTime.of(2024, 1, 15, 6, 42, 29, 400000000))
    )

    val json2 =
      """{"name": "test.fits", "instrument": "GMOS-N", "observation_type": "OBJECT", "ut_datetime": "2024-01-15 06:42:29"}"""
    val result2 = decode[GoaSummaryRecord](json2.stripMargin)
    assert(result2.isRight)
    assertEquals(result2.toOption.get.utDateTime, Some(LocalDateTime.of(2024, 1, 15, 6, 42, 29)))
