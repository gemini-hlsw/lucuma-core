...
trait ArbPosAngle {
  ...
  implicit val fixedPosAngleArb: Arbitrary[PosAngle.Fixed] = Arbitrary {
    for {
      a <- arbitrary[Angle]
  ...
  implicit val allowFlipPosAngleArb: Arbitrary[PosAngle.AllowFlip] = Arbitrary {
    for {
      a <- arbitrary[Angle]
  ...
  implicit val parallacticOverridePosAngleArb: Arbitrary[PosAngle.ParallacticOverride] = Arbitrary {
    for {
      a <- arbitrary[Angle]
  ...
  implicit val posAngleArb: Arbitrary[PosAngle] = Arbitrary {
    for {
