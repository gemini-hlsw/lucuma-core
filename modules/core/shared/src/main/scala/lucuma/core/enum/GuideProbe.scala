...
/**
 * Enumerated type for Guiding probes
...
object GuideProbe {
  ...
  implicit val GuideProbeEnumerated: Enumerated[GuideProbe] =
    new Enumerated[GuideProbe] {
      def all                                           = GuideProbe.all
      def tag(a: GuideProbe)                            = a.tag
      override def unsafeFromTag(s: String): GuideProbe =
        GuideProbe.unsafeFromTag(s)
    }
