...
Standard 2-way optics deal with **invertible** mappings. `Iso[A, B]` says that `A` and `B` are equal, so round-trips in either direction are identities. `Prism[A, B]` says that there is some _subset_ of `A` that is equal to `B`.
...
A `Format` is a weaker `Prism` where a _subset_ of `A` forms a split epi with `B`. Every `Prism` is a `Format` where the split epi happens to be an `Iso`; and every `SplitEpi` forms a `Prism` where the subset of `A` is `A` itself.

A `ValidFormat` is a reformulation of a `Format`, where elements of `A` that don't map to an element of `B` return an element of `E` instead.
