...
private[catalog] trait SpectralTypeParsers:
  ...
  // Temperature subclass: digit, optionally followed by decimal
  ...
  /**
   * Optional spectral peculiarity suffixes to skip: n (nebular), w (weak), p (peculiar). When "n"
   * suffix is present after luminosity class, it can affect temperature parsing (some systems treat
   * O9.7n as O9n, ignoring fractional subclass)
   */
  ...
  val tempRange: Parser[List[String]] =
    (tempClass ~ (separator ~ (tempClass | partialTempClass)).rep0 ~ (tempClassNoMod | partialTempClass).?)
      .map { case ((first, rest), trailing) =>
        ...
        // if second starts with digit, prepend first letter
        if (classes.length > 1) {
