 ...
 * Handles main sequence stars, giants, supergiants, white dwarfs, and subdwarfs.
 */
trait SpectralTypeParsers:
  ...
  /** Optional temperature subclass: digit, optionally followed by .5 */
  private val tempSubclass: Parser0[String] =
    (digit ~ string(".5").?).string
  ...
  /**
   * Temperature class: e.g., O9, G2, K3.5, M5+
   * Pattern: [OBAFGKMLTY][0-9]?(.5)?[+-]?
   */
  ...
  /**
   * Luminosity class: e.g., V, III, Ia, IVb
   * Pattern: (I|II|III|IV|V|VI|VII|VIII)[ab]?
   */
  ...
  /**
   * Temperature range: e.g., G8, G8/K0, M8-9, M2/3
   * Returns list of normalized temperature classes
   */
  val tempRange: Parser[List[String]] =
    (tempClass ~ (separator ~ (tempClass | partialTempClass)).rep0).map { case (first, rest) =>
  ...
  /**
   * Luminosity range: e.g., V, IV/V, IIIb
   * Returns list of normalized luminosity classes
   */
  val lumRange: Parser[List[String]] =
    (lumClass ~ (separator ~ lumClass).rep0).map { case (first, rest) =>
      val classes = first :: rest.map(_._2)
  ...
  /**
   * White dwarf spectral type: e.g., DA3.5, DBAP3, DC
   * Pattern: D[ABCGKMOQPXZ]*<temperature>?
   * Temperature can include decimal point
   */
  val whiteDwarf: Parser[(List[String], List[String])] =
    (char('D') ~ charIn("ABCGKMOQPXZ").rep0 ~ (digit.rep ~ (char('.') ~ digit.rep).?).string.?).map {
      case ((_, letters), tempOpt) =>
  ...
  /**
   * Subdwarf spectral type: e.g., sdO2, sdG, sdB1, sdBN0
   * Pattern: sd<anything that looks like a temperature class>?
   * Uses lenient parsing to handle non-standard subdwarf classifications
   */
  val subdwarf: Parser[(List[String], List[String])] =
    (string("sd") *> lenientTempClass.?).map {
      case Some(temp) => (List("sd"), List(temp))
      case None       => (List("sd"), List.empty)
    }.withContext("subdwarf")

  /**
   * Main sequence spectral type: e.g., G2V, K3III, G8/K0III
   * Pattern: <tempRange><lumRange>?
   */
  val mainSequence: Parser[(List[String], List[String])] =
    (tempRange ~ lumRange.?).map {
      case (temps, Some(lums)) => (lums, temps)
      case (temps, None)       => (List.empty, temps)
    }.withContext("main sequence")

  /**
   * Complete spectral type parser.
   * Tries white dwarf, subdwarf, then main sequence in order.
   */
