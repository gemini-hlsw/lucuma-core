...
object StellarLibraryParameters:
  ...
  /**
   * Ordered list from Python's match_sed_stars.dat.
   * The order matters for tie-breaking when multiple spectra have identical scores.
   * Python's sort is stable, so the first match in file order wins on ties.
   */
  private val librarySpectraOrdered: List[(StellarLibrarySpectrum, (List[String], List[String]))] = List(
  ...
  /**
   * Map of stellar library spectra to their physical params.
   * Uses explicit (lc, tc) mappings from Python's match_sed_stars.dat.
   */
