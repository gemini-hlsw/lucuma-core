...
/**
 * Stellar physics calculations for matching spectral types to library SEDs.
 * Based on the Python reference implementation (match_sed.py by Andrew Stephens).
 *
 * References:
...
object StellarPhysics:
  ...
  /**
   * Convert a spectral class string to a spectral class code.
   * O0 = 0, B0 = 10, A0 = 20, F0 = 30, G0 = 40, K0 = 50, M0 = 60, etc.
  ...
  def spectralClassCode(spectralClass: String): Option[Double] =
    ...
    val letterConversions = Map(
      'O' -> 0.0, 'B' -> 10.0, 'A' -> 20.0, 'F' -> 30.0,
      'G' -> 40.0, 'K' -> 50.0, 'M' -> 60.0, 'L' -> 70.0,
      'T' -> 80.0, 'Y' -> 90.0
    )
    ...
    letterConversions.get(letter).flatMap { baseCode =>
      ...
      val modifier = if spectralClass.contains('+') then 0.25
  ...
  /**
   * Calculate effective temperature from luminosity and temperature classes.
   * Uses the polynomial relation from Malkov et al, 2020, RAA, 20, 139.
   * This is the relation for giants (III) but works well for all luminosity classes I-V.
   */
  def calculateTemperature(
    ...
    val temperatures = temperatureClasses.flatMap { tc =>
      spectralClassCode(tc).flatMap { scc =>
        if scc >= 70.0 then
          None // No support for brown dwarfs (L, T, Y) yet
        else
          // Polynomial from Malkov et al. 2020
          val logTEff = 5.07073 - 7.57056e-2 * scc + 1.47089e-3 * scc * scc - 1.03905e-5 * scc * scc * scc
  ...
  /**
   * Gravity interpolation table from Straizys & Kuriliene, 1981, Ap&SS, 80, 353S.
   * Extended with data from Malkov et al, 2020, RAA, 20, 139.
   */
  private val gravityTable: List[(Double, Map[String, Double])] = List(
    (1.0,  Map("V" -> 3.90, "IV" -> 3.85, "III" -> 3.80, "II" -> 4.05, "Ib" -> 4.00, "Iab" -> 3.90, "Ia" -> 3.80, "sd" -> 4.5)),
    (5.0,  Map("V" -> 3.90, "IV" -> 3.86, "III" -> 3.82, "II" -> 3.76, "Ib" -> 3.74, "Iab" -> 3.69, "Ia" -> 3.57)),
    (10.0, Map("V" -> 4.00, "IV" -> 3.88, "III" -> 3.74, "II" -> 3.39, "Ib" -> 3.27, "Iab" -> 3.19, "Ia" -> 3.05, "sd" -> 4.5)),
    (20.0, Map("V" -> 4.07, "IV" -> 3.91, "III" -> 3.75, "II" -> 2.85, "Ib" -> 2.23, "Iab" -> 2.01, "Ia" -> 1.81, "sd" -> 4.5)),
    (30.0, Map("V" -> 4.28, "IV" -> 4.05, "III" -> 3.83, "II" -> 2.67, "Ib" -> 2.00, "Iab" -> 1.51, "Ia" -> 1.25, "sd" -> 4.0)),
    (40.0, Map("V" -> 4.39, "IV" -> 3.84, "III" -> 3.35, "II" -> 2.29, "Ib" -> 1.62, "Iab" -> 0.95, "Ia" -> 0.72, "sd" -> 3.5)),
    (50.0, Map("V" -> 4.57, "IV" -> 3.57, "III" -> 2.89, "II" -> 1.74, "Ib" -> 1.20, "Iab" -> 0.54, "Ia" -> 0.25, "sd" -> 3.0)),
    (60.0, Map("V" -> 4.61, "IV" -> 3.35, "III" -> 1.63, "II" -> 1.01, "Ib" -> 0.61, "Iab" -> 0.30, "Ia" -> 0.00, "sd" -> 3.0)),
    (69.0, Map("V" -> 5.32, "IV" -> 3.15, "III" -> -0.10, "II" -> -0.30, "Ib" -> -0.50, "Iab" -> -0.60, "Ia" -> -0.70))
  )

  /**
   * Calculate log(g) from luminosity and temperature classes.
   * Uses interpolation from Straizys & Kuriliene, 1981, Ap&SS, 80, 353S.
   */
  def calculateGravity(
    ...
    yield
      // Normalize luminosity class
      val normalizedLC = lc match
        case "I" => "Iab" // Default I to Iab
        case "VI" => "sd" // VI is subdwarf
        case s if s.matches("I+[ab]") => s.take(s.indexWhere(c => c == 'a' || c == 'b') + 1)
        case s if s.endsWith("a") || s.endsWith("b") => s.dropRight(1) // Drop subclass
        case s => s

      interpolateGravity(scc, normalizedLC)

    if allLogG.isEmpty then None
    else Some(BigDecimal(allLogG.sum / allLogG.length).setScale(3, BigDecimal.RoundingMode.HALF_UP).toDouble)
